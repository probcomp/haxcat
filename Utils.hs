...
mapZipWith :: (Ord k) => (Maybe v1 -> Maybe v2 -> Maybe v3)
           -> M.Map k v1 -> M.Map k v2 -> M.Map k v3
mapZipWith f m1 m2 = M.map fromJust
                   $ M.filter isJust
                   $ M.map (uncurry f) $ mapZipWithHoles m1 m2

mapZipWithHoles :: (Ord k) => M.Map k v1 -> M.Map k v2
                -> M.Map k (Maybe v1, Maybe v2)
mapZipWithHoles m1 m2 = M.fromAscList $ go (M.toAscList m1) (M.toAscList m2)
    where
      go [] [] = []
