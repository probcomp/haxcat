...
- Refactorings:
  ...
  - Could split view sampling into components
...
- I am somewhat confused about sampling vs prediction.
  ...
  - I should also be able to define predict as sample+incorporate
    - May tweak sampling to take the row id and check whether it was
      already present
    - Corrent incorporate assumes the incorporee is fresh, but could
...
- Is there any kind of test that would detect the bug of forgetting to incorporate?
...
- A test for NIG Normal: For all parameter values and all run lengths, it induces
  the same distribution as the uncollapsed version.
  - What exactly does this exercise?
- Benefit: geweke test n x m
  ...
  - Problems with chi2 testing:
    - the test in the statistics package doesn't return the p value
  ...
  - Could also do K-S with respect to the Ord instance (which is more sensitive?)
