...
- Draft collecting a data table
  - Frames data loading is kooky: it guesses the data's types by
    reading the dataset _at compile time_.
    - I guess that's not too bad; and can presumably be overridden at need
  - Frames also depends on base >= 4.7 && < 4.9
