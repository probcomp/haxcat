...
- Import and possibly merge Taylor's draft commits
  - Taylor's suggested numerical improvement over log (alpha/(alpha+beta))
    - Likewise for categorical.
  - Check chat for Taylor's other suggestions
- Flush NoStat?
...
Cleanup:
- Flush (or delimit?) DPMM; flush DPMM-specific Utils; reorient
  Test.hs to test Crosscat
- Alternative: Rewrite DPMM to use the pieces defined in Models;
  re-test it
- Get permission from Baxter to relicense as Apache or whatever the
  crosscat license is?
