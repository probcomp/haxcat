...
- Refactorings:
  ...
  - Could migrate the Map ColID Column out to the Crosscat object.
    ...
    - define a cc_row_{un,re}inc for updating the suff stats
      - reinc needs all the per-view cluster ids
    - change view_row_step to accept the column map (including a spec
      of which columns are relevant to this view!) and return the new
      ClusterID
    - change RowSweep to define cc_row_step, which
      - unincorporates from the cc-wide suff stats
      - runs the view_row_steps
      - reincorporates with the new cluster ids
  - Might be better to introduce a new object, e.g. CRPMixture, which
    just knows the CRP, counts, and partition, and split View into a
    CRPMixture and the current column map.
...
- Benefit: profile inference
  ...
  - Persistent memory usage is good: 400k
    - But, around 100k of it is unboxed doubles?
...
- For simulation, would want:
  - view_cluster_predict :: RowID -> View -> RVar (ClusterID, View)
       which is a view_cluster_sample and a view_cluster_reinc if the
       row id is fresh, or produces the old value if it is stale
  - view_predict :: RowID -> View -> RVar (M.Map ColID Double, View)
    which does a view_sample and a view_reinc if the row id is fresh
    (and does what if it is stale? can't return the current value without
    access to the dataset, which the view doesn't actually store)
  - I also have the problem that I may want to have partially
    incorporated views: ones whose partition is present and
    incorporated, but whose component suff stats are not.  Prediction
    is well-defined in this case: use the existing cluster assignment,
    make up an end value, and incorporate it into the suffstats.  The
    trouble is detecting this state; for which I seem to want to give
    views read access to the set of actual data items that have been
    incorporated (as a Map RowID Row, presumably)
- Plan of attack:
  ...
  - Split view sampling into components
    - Names: view{_cluster,_row,}_sample
  - Add a map of incorporated rows to View (which will be static
    ...
    - Funny consequence: inference wouldn't need to accept the dataset
      ...
      - May be paying with extra map lookups if I do that
  - Define _predict for CRPMixture, view, and crosscat-rowwise
    - I think I still need two names for the view and cc versions, to
      distinguish prediction to the datum or just the latent
  ...
  - Note: I can Geweke test the NIGNormal code separately, on one
    instance.
